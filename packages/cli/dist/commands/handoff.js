...
import { getCurrentStage, getStage, validateStageOutputs, generateHandoff, saveHandoff, handoffExists, getAICallLogs, } from '@ax-templates/core';
...
async function executeHandoff(stageId, options = {}) {
    // Determine stage
...
async function gatherInteractiveOptions(stageId, completedOutputs) {
...
async function gatherQuickOptions(stageId, completedOutputs) {
    const stage = getStage(stageId);
    const aiLogs = getAICallLogs();
    // Convert AI logs to handoff format
    const aiCalls = aiLogs
