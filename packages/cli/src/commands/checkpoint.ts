...
export const checkpointCommand = new Command('checkpoint')
  ...
  .action(async (action?: string, checkpointId?: string, options?: { message?: string; stage?: string }) => {
...
async function executeCreateCheckpoint(options: { message?: string; stage?: string } = {}) {
  ...
  if (!description) {
    const answers = await inquirer.prompt([{
      ...
      validate: (input: string) => input.trim() ? true : '설명을 입력하세요.',
    }]);
...
async function executeListCheckpoints(stageId?: string) {
  console.log();
...
function displayCheckpoints(checkpoints: Checkpoint[]) {
...
async function executeShowCheckpoint(checkpointId?: string) {
  if (!checkpointId) {
