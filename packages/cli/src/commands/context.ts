...
async function executeUpdate(options: { percent?: number; tokens?: number; max?: number }) {
  console.log();
...
async function executeSnapshot(options: { stage?: string }) {
  console.log();
...
function createContextBar(remainingPercent: number, width: number = 20): string {
...
function getThresholdColor(threshold: string): (text: string) => string {
  switch (threshold) {
    case 'critical':
      return chalk.red;
    case 'action':
...
function getThresholdBorderColor(threshold: string): string {
  switch (threshold) {
    case 'critical':
      return 'red';
    case 'action':
