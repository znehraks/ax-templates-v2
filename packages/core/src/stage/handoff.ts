 ...
 */
export function generateHandoff(options: GenerateHandoffOptions): string {
  const {
    stageId,
  ...
  const aiCallsTable = aiCalls.length > 0
    ? aiCalls.map(c =>
        `| ${c.model} | ${c.time} | ${c.prompt} | ${c.result} | ${c.status === 'success' ? '✅' : '❌'} |`
      ).join('\n')
    : '| (AI 호출 없음) | - | - | - | - |';
 ...
 */
export function parseHandoff(content: string): Partial<HandoffDocument> {
  const result: Partial<HandoffDocument> = {};
