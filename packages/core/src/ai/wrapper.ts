 ...
 */
function getAILogPath(projectDir: string = process.cwd()): string {
 ...
 */
export function getAICallLogs(projectDir: string = process.cwd()): AICallLog[] {
 ...
 * Logs an AI call
 */
export function logAICall(log: Omit<AICallLog, 'id'>, projectDir: string = process.cwd()): AICallLog {
  const logPath = getAILogPath(projectDir);
 ...
 */
export function updateAICallLog(
  id: string,
 ...
 */
export function getTmuxSessionStatus(sessionName: string): TmuxSessionStatus {
  const result: TmuxSessionStatus = {
    name: sessionName,
    exists: false,
    attached: false,
    windowCount: 0,
 ...
 */
export function ensureTmuxSession(sessionName: string): boolean {
 ...
 */
export function checkAIAvailability(): Record<AIProvider, boolean> {
  return {
...
export async function executeAICall(
  ...
  try {
    ...
    execSync(
      `tmux send-keys -t "${sessionName}" '${provider} "${escapedPrompt}" 2>&1 | tee ${tempOutput}; tmux wait-for -S ${channel}' Enter`,
      { stdio: 'pipe' }
    );
 ...
 */
export function getWrapperScriptPath(
  provider: AIProvider,
  projectDir: string = process.cwd()
): string {
 ...
 */
export async function executeViaWrapper(
  provider: AIProvider,
  prompt: string,
  timeout?: number,
  projectDir: string = process.cwd()
