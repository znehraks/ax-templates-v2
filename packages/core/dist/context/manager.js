...
import { getCurrentStage } from '../stage/index.js';
// ============================================
// Constants
// ============================================
 ...
 */
export function getContextDir(projectDir = process.cwd()) {
 ...
 */
export function getContextState(projectDir = process.cwd()) {
 ...
 */
export function updateContextState(state, projectDir = process.cwd()) {
 ...
 */
export function calculateThreshold(usagePercent, projectDir = process.cwd()) {
    const config = loadConfig({ projectDir });
 ...
 */
export function getRemainingContext(projectDir = process.cwd()) {
 ...
 */
export function getRecommendedActions(projectDir = process.cwd()) {
    const state = getContextState(projectDir);
    if (!state)
        return [];
    const config = loadConfig({ projectDir });
 ...
 */
export function formatContextStatus(projectDir = process.cwd()) {
    const state = getContextState(projectDir);
    if (!state) {
 ...
 */
export function generateSnapshotId(stageId) {
 ...
 */
export function createSnapshot(trigger, options = {}, projectDir = process.cwd()) {
    const contextDir = getContextDir(projectDir);
    if (!existsSync(contextDir)) {
        mkdirSync(contextDir, { recursive: true });
    }
 ...
 */
export function listSnapshots(projectDir = process.cwd()) {
    const contextDir = getContextDir(projectDir);
    if (!existsSync(contextDir)) {
        return [];
    }
 ...
 */
export function getLatestSnapshot(projectDir = process.cwd()) {
 ...
 */
export function getTaskLog(projectDir = process.cwd()) {
 ...
 */
export function logTaskCompletion(description, stageId, projectDir = process.cwd()) {
    const contextDir = getContextDir(projectDir);
    if (!existsSync(contextDir)) {
        mkdirSync(contextDir, { recursive: true });
    }
 ...
 */
export function getTasksSinceLastSnapshot(projectDir = process.cwd()) {
    const log = getTaskLog(projectDir);
    const lastSnapshot = getLatestSnapshot(projectDir);
    if (!lastSnapshot) {
        return log.length;
    }
    return log.filter(t => new Date(t.completedAt) > new Date(lastSnapshot.createdAt)).length;
}
// ============================================
