...
import { loadConfig, loadPipelineConfig } from '../config/index.js';
// ============================================
// Constants
// ============================================
 ...
 */
export function getProgressPath(projectDir = process.cwd()) {
 ...
 */
export function loadProgress(projectDir = process.cwd()) {
 ...
 */
export function saveProgress(progress, projectDir = process.cwd()) {
    const progressPath = getProgressPath(projectDir);
 ...
 */
export function updateStageProgress(stageId, update, projectDir = process.cwd()) {
 ...
 */
export function getAllStages(projectDir = process.cwd()) {
 ...
 * Gets a stage by ID
 */
export function getStage(stageId, projectDir = process.cwd()) {
 ...
 */
export function getCurrentStage(projectDir = process.cwd()) {
    const progress = loadProgress(projectDir);
 ...
 */
export function getNextStage(currentStageId, projectDir = process.cwd()) {
    const stages = getAllStages(projectDir);
 ...
 */
export function getPreviousStage(currentStageId, projectDir = process.cwd()) {
    const stages = getAllStages(projectDir);
    const currentIndex = stages.findIndex(s => s.id === currentStageId);
    if (currentIndex <= 0) {
        return null;
 ...
 */
export function getFirstStage(projectDir = process.cwd()) {
    const stages = getAllStages(projectDir);
    return stages[0];
}
// ============================================
 ...
 */
export function validateStageInputs(stageId, projectDir = process.cwd()) {
 ...
 */
export function validateStageOutputs(stageId, projectDir = process.cwd()) {
    const stage = getStage(stageId, projectDir);
    if (!stage) {
        return { valid: false, missing: [], present: [] };
    }
 ...
 */
export function validateTransition(fromStageId, toStageId, projectDir = process.cwd()) {
    const errors = [];
    const warnings = [];
    const pipeline = loadPipelineConfig(projectDir);
 ...
 * Starts a stage
 */
export function startStage(stageId, projectDir = process.cwd()) {
    const stage = getStage(stageId, projectDir);
    if (!stage) {
 ...
 * Completes a stage
 */
export function completeStage(stageId, outputs = [], checkpointId, projectDir = process.cwd()) {
    const progress = loadProgress(projectDir);
    const stageProgress = {
        ...progress.stages[stageId],
        stageId,
 ...
 * Fails a stage
 */
export function failStage(stageId, error, projectDir = process.cwd()) {
    const progress = loadProgress(projectDir);
    const stageProgress = {
        ...progress.stages[stageId],
        stageId,
 ...
 */
export function getStageSummary(projectDir = process.cwd()) {
    const progress = loadProgress(projectDir);
    const stages = getAllStages(projectDir);
    let completed = 0;
    let inProgress = 0;
